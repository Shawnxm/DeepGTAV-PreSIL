...
void ObjectDetection::initCollection(UINT camWidth, UINT camHeight, bool exportEVE, int startIndex) {
	...
	m_timestampsFile = baseFolder + "timestamps.txt";  // xumiao add
    ...
    initVehicleLookup();
	// log("[xumiao, objectdetection] after initVehicleLookup\n", true);
    //Setup LiDAR before collecting
    setupLiDAR();
	// log("[xumiao, objectdetection] after setupLiDAR\n", true);
    m_initialized = true;
}
...
FrameObjectInfo ObjectDetection::setDepthAndStencil(bool prevDepth, float* pDepth, uint8_t* pStencil) {
	if (prevDepth) {
        m_pDepth = pDepth;
        m_pStencil = pStencil;
		// log("[xumiao, objectdetection] bad entry!\n", true);
    }
    else {
		// log("[xumiao, objectdetection] before setFilenames\n", true);
		setFilenames();
		// log("[xumiao, objectdetection] after setFilenames\n", true);
    }

    if (lidar_initialized) setDepthBuffer(prevDepth); //  xumiao comment
	// log("[xumiao, objectdetection] 01\n", true);
    if (lidar_initialized) setStencilBuffer(); //  xumiao comment
	// log("[xumiao, objectdetection] 02\n", true);
...
FrameObjectInfo ObjectDetection::generateMessage(float* pDepth, uint8_t* pStencil, int entityID) {
	...
	setPosition();
	outputRealSpeed();
	// log("[xumiao, objectdetection] 4\n", true);
	setDepthAndStencil();
    //Need to set peds list first for integrating peds on bikes
	// log("[xumiao, objectdetection] 5\n", true);
	setPedsList();
	// log("[xumiao, objectdetection] 6\n", true);
	setVehiclesList();
	// log("[xumiao, objectdetection] 7\n", true);
	setSpeed();
	// log("[xumiao, objectdetection] 8\n", true);
	setYawRate();
	// log("[xumiao, objectdetection] 9\n", true);
	setTime();
	// log("[xumiao, objectdetection] 10\n", true);
	setFocalLength();
    log("After focalLength");
	// log("[xumiao, objectdetection] 11\n", true);

    //Update 2D bboxes, and create segmentation of stencil values
    processSegmentation2D(); //  xumiao comment
	// log("[xumiao, objectdetection] 2D\n", true);
    processSegmentation3D(); //  xumiao comment
	// log("[xumiao, objectdetection] 3D\n", true);
    processOcclusion(); //  xumiao comment
	// log("[xumiao, objectdetection] occ\n", true);
	...
	// log("[xumiao, objectdetection] after generate message\n", true);
...
void ObjectDetection::processSegmentation3D() {
	...
	// log("[xumiao, objectdetection] 3D1\n", true);
	for (int j = 0; j < s_camParams.height; ++j) {
        for (int i = 0; i < s_camParams.width; ++i) {
	...
	// log("[xumiao, objectdetection] 3D2\n", true);
	...
	// log("[xumiao, objectdetection] 3D3\n", true);
	for (int j = 0; j < s_camParams.height; ++j) {
        for (int i = 0; i < s_camParams.width; ++i) {
            uint8_t stencilVal = m_pStencil[j * s_camParams.width + i];
...
void ObjectDetection::setFilenames() {
    ...
    m_depthFilename = getStandardFilename("depth", ".bin"); //  xumiao comment
    m_stencilFilename = getStandardFilename("stencil", ".raw"); //  xumiao comment
	...
	m_oxtsFilename = getStandardFilename("oxts", ".txt");  // xumiao add

    //TODO - Why are two seg images being printed (there are some minor differences in images it appears)
    m_segImgFilename = getStandardFilename("segImage", ".png"); //  xumiao comment
    m_instSegFilename = getStandardFilename("instSeg", ".png"); //  xumiao comment
    m_instSegImgFilename = getStandardFilename("instSegImage", ".png"); //  xumiao comment
...
void ObjectDetection::setStencilBuffer() {
	...
	// log("[xumiao, objectdetection] After writing stencil buffer\n", true);
    for (int j = 0; j < s_camParams.height; ++j) {
        for (int i = 0; i < s_camParams.width; ++i) {
...
void ObjectDetection::setDepthBuffer(bool prevDepth) {
	...
	// log("[xumiao, objectdetection] to set depth buffer\n", true);
    ...
    if (OUTPUT_DM_POINTCLOUD || OUTPUT_GROUND_PIXELS) {
		...
		// log("[xumiao, objectdetection] depth2\n", true);
        ...
        if (OUTPUT_DM_POINTCLOUD) {
			...
			// log("[xumiao, objectdetection] depth3\n", true);
        }
    }
}
...
void ObjectDetection::exportOxts() {  // xumiao add
	FILE* f = fopen(m_oxtsFilename.c_str(), "w");
	std::ostringstream oss;

	oss << m_curFrame.kittiWorldPos.x << " " << m_curFrame.kittiWorldPos.y << " " << m_curFrame.kittiWorldPos.z << " "
		<< m_curFrame.roll << " " << m_curFrame.pitch << " " << m_curFrame.heading;
...
void ObjectDetection::exportDetections(FrameObjectInfo fObjInfo, ObjEntity* vPerspective) {
    ...
    exportEntities(fObjInfo.vehicles, oss, false, false, true, OBJECT_MAX_DIST, 1, 1);  // xumiao, true, OBJECT_MAX_DIST, 1, 1
    exportEntities(fObjInfo.peds, oss, false, false, true, OBJECT_MAX_DIST, 1, 1);  // xumiao, true, OBJECT_MAX_DIST, 1, 1
